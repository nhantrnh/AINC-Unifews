...
class ConvThr(nn.Module):
    def __init__(self, *args, thr_a, thr_w, thr_mode='adaptive', fa_alpha=0.5, **kwargs):
        ...
        # Node-Adaptive Alpha MLP: chỉ khởi tạo khi fa_alpha < 0
        if self.fa_alpha < 0.0:
...
class GCNConvRaw(pyg_nn.GCNConv):
    def __init__(self, *args, **kwargs):
        ...
        kwargs.pop("rnorm", None)
        kwargs.pop("diag", None)
        kwargs.pop("alpha", None)
        kwargs.pop("fa_alpha", None)
        kwargs.pop("thr_a", None)
        kwargs.pop("thr_w", None)
        kwargs.pop("thr_mode", None)
...
class GCNConvThr(ConvThr, GCNConvRaw):
    def __init__(self, *args, thr_a, thr_w, fa_alpha=0.5, **kwargs):
        super(GCNConvThr, self).__init__(*args, thr_a=thr_a, thr_w=thr_w, fa_alpha=fa_alpha, **kwargs)
        self.prune_lst = [self.lin]
    ...
    def prune_on_msg(self, module, inputs, output):
        ...
        if self.scheme_a in ['pruneall', 'pruneinc']:
            ...
            # Thay vì dùng norm_feat_msg (chỉ dựa vào message), 
            # ta dùng fa_score (đã kết hợp Cosine Similarity)
            
            importance_score = self.fa_score # Lấy từ hàm message đã tính
            
            if self.threshold_mode == 'fixed':
                mask_cmp = importance_score < self.threshold_a
            else:
                # Tính ngưỡng động dựa trên trung bình fa_score
                avg_importance = importance_score.mean()
                mask_cmp = importance_score < self.threshold_a * avg_importance
    ...
    def message(self, x_i, x_j, edge_weight):
        # Message gốc và Structural Magnitude
        msg = x_j if edge_weight is None else edge_weight.view(-1, 1) * x_j
        struct_mag = torch.norm(msg, dim=1)

        # Chế độ 1: UNIFEWS gốc (fa_alpha == 1.0)
        if self.fa_alpha == 1.0:
            self.fa_score = struct_mag
            return msg

        # Tính Cosine Similarity cho chế độ 2 và 3
        cos_sim = F.cosine_similarity(x_i, x_j, dim=1, eps=1e-8)
        cos_sim = (cos_sim + 1.0) / 2.0
        mean_struct = struct_mag.mean().detach()
        mean_cos = cos_sim.mean().detach() + 1e-8
        cos_sim_scaled = cos_sim * (mean_struct / mean_cos)

        # Chế độ 2: FA-UNIFEWS tĩnh (fa_alpha >= 0)
        if self.fa_alpha >= 0.0:
            self.fa_score = self.fa_alpha * struct_mag + (1.0 - self.fa_alpha) * cos_sim_scaled
            return msg

        # Chế độ 3: Node-Adaptive (fa_alpha < 0)
        alpha_i = self.alpha_mlp(x_i).squeeze(-1)
        self.fa_score = alpha_i * struct_mag + (1.0 - alpha_i) * cos_sim_scaled
        msg = msg * (0.5 + alpha_i.unsqueeze(-1) * 0.5)
        return msg
...
class GATv2ConvRnd(ConvThr, GATv2ConvRaw):
    ...
    def forward(self, x: Tensor, edge_index: Adj,
        ...
        # Pass alpha via edge_attr slot — PyG generates typed propagate with
        # (edge_index, x, edge_attr, size) from parent GATv2Conv's message sig.
        out = self.propagate(edge_index, x=(x_l, x_r), edge_attr=alpha, size=None)
        if self.concat:
...
class GATv2ConvThr(ConvThr, GATv2ConvRaw):
    def __init__(self, *args, thr_a, thr_w, fa_alpha=0.5, **kwargs):
        super(GATv2ConvThr, self).__init__(*args, thr_a=thr_a, thr_w=thr_w, fa_alpha=fa_alpha, **kwargs)
        self.prune_lst = [self.lin_l, self.lin_r]

    def forward(self, x: Tensor, edge_input,
                edge_attr: OptTensor = None,
                node_lock: OptTensor = None, verbose: bool = False):
        """Shape:
            x: [n, F_in]
            ...
            x_l: [n, H, F_out//H]
        """
        # Unpack tuple if needed (GNNThr passes (edge_index, edge_weight))
        if isinstance(edge_input, tuple):
            edge_index, _edge_weight = edge_input
        else:
            edge_index = edge_input
        H, C = self.heads, self.out_channels
        x_l: OptTensor = None
        x_r: OptTensor = None
        assert x.dim() == 2
        ...
        if edge_index.device.type == 'cuda':
            with torch.cuda.device(edge_index.device):
                torch.cuda.empty_cache()
        # Return as tuple for GNNThr compatibility
    ...
    def edge_update(self, x_j: Tensor, x_i: Tensor, edge_attr: OptTensor,
        ...
        if self.scheme_a in ['pruneall', 'pruneinc']:
            ...
            struct_mag = torch.norm(x_j, dim=[1,2])

            # Chế độ 1: UNIFEWS gốc
            if self.fa_alpha == 1.0:
                fa_score = struct_mag
            else:
                # Cosine similarity (flatten multi-head)
                ...
                # Chế độ 2: FA-UNIFEWS tĩnh
                if self.fa_alpha >= 0.0:
                    fa_score = self.fa_alpha * struct_mag + (1.0 - self.fa_alpha) * cos_sim_scaled
                # Chế độ 3: Node-Adaptive
                else:
                    alpha_i = self.alpha_mlp(x_i.reshape(x_i.size(0), -1)).squeeze(-1)
                    fa_score = alpha_i * struct_mag + (1.0 - alpha_i) * cos_sim_scaled

            avg_fa = fa_score.mean()
            mask_cmp = fa_score < self.threshold_a * avg_fa
            mask_0 = torch.logical_or(mask_0, mask_cmp)
            mask_0[self.idx_lock] = False
            alpha[mask_0] = 0
            self.idx_keep = torch.where(~mask_0)[0]
    ...
    def message(self, x_j: Tensor, edge_attr: Tensor) -> Tensor:
        # edge_attr carries attention alpha (passed through PyG's typed propagate)
        return x_j * edge_attr.unsqueeze(-1)

    @classmethod
    def cnt_flops(cls, module, input, output):
        x_in, _ = input
        x_out, edge_out = output
        if isinstance(edge_out, tuple):
            edge_index, _ = edge_out
        else:
            edge_index = edge_out
        f_in, f_h, f_c = x_in.shape[-1], module.heads, module.out_channels
        n, m = x_in.shape[0], edge_index.shape[1]
...
class GCNIIConvThr(ConvThr, GCNIIConvRaw):
    ...
    def prune_on_msg(self, module, inputs, output):
        if self.scheme_a in ['pruneall', 'pruneinc']:
            ...
            importance_score = self.fa_score
...
class SAGEConvThr(ConvThr, SAGEConvRaw):
    def __init__(self, *args, thr_a, thr_w, fa_alpha=0.5, **kwargs):
        super(SAGEConvThr, self).__init__(*args, thr_a=thr_a, thr_w=thr_w, fa_alpha=fa_alpha, **kwargs)
