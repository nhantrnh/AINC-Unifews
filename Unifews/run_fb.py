...
def get_device(dev_id):
    if dev_id >= 0 and torch.cuda.is_available():
...
def to_device(x, device):
    """Move tensor or tuple of tensors to device."""
...
parser.add_argument('--fa_alpha', type=float, default=0.5, help='FA-alpha for FA-UNIFEWS (default 0.5)')
...
if args.algo.split('_')[0] in ['gcn2']:
    model = models.SandwitchThr(nlayer=args.layer, nfeat=nfeat, nhidden=args.hidden, nclass=nclass,
                        thr_a=args.thr_a, thr_w=args.thr_w, fa_alpha=args.fa_alpha, dropout=args.dropout, layer=args.algo)
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, threshold=1e-4, patience=15)
...
def train(x, edge_idx, y, idx_split, epoch, verbose=False):
    ...
    edge_idx = to_device(edge_idx, device)
    stopwatch.reset()

    stopwatch.start()
    optimizer.zero_grad()
    output = model(x, edge_idx, node_lock=torch.Tensor([]).to(device), verbose=verbose)[idx_split]
...
for epoch in range(1, args.epochs+1):
    ...
    if verbose:
        res = f"Epoch:{epoch:04d} | train loss:{loss_train:.4f}, val acc:{acc_val:.4f}, time:{time_tol.val:.4f}"
        if logger.lvl_log > 1:
            logger.print(res)
...
# Compute MACs once after training (not per-epoch)
macs_train = cal_flops(x=feat['train'], edge_idx=adj['train'], idx_split=idx['train'])
macs_tol = metric.Accumulator()
macs_tol.update(macs_train)
